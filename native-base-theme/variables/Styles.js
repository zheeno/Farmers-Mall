import { StyleSheet, Dimensions } from 'react-native';

export const styles = StyleSheet.create({
  introHeader: {
    fontSize: 40,
    fontWeight: '800',
    color: '#FFF',
  },

  introHeader_2: {
    fontSize: 30,
    fontWeight: '800',
    color: '#FFF',
  },

  maskDarkSlight: {
    backgroundColor: 'rgba(6, 6, 6, 0.39)',
  },

  maskDarkLight: {
    backgroundColor: 'rgba(6, 6, 6, 0.2)',
  },

  maskDark: {
    backgroundColor: 'rgba(6, 6, 6, 0.50)',
  },

  whiteText: {
    color: '#FFF',
  },

  greenText: {
    color: 'rgb(14, 46, 47)',
  },

  greyText: {
    color: '#cccccc',
  },

  centerText: {
    textAlign: 'center',
  },

  bgWhite: {
    backgroundColor: "white"
  },

  myButt: {
    height: "100%",
    alignItems: "center",
    justifyContent: "center"
  },

  bgLeafGreen: {
    backgroundColor: 'rgb(14, 46, 47)',
  },

  bgBrickRed: {
    backgroundColor: '#b80514',
  },

  bgLeafGreenSlight: {
    backgroundColor: 'rgba(14, 46, 47, 0.85)',
  },

  bgGrey: {
    backgroundColor: '#e0e0e0',
  },

  largeRoundBtn: {
    height: 60,
    width: 60,
    borderRadius: 70,
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: 'rgb(14, 46, 47)',
    color: '#FFF',
  },

  fs_14: {
    fontSize: 14
  },

  card: { marginLeft: 20, marginRight: 20, marginTop: 30, borderRadius: 20, backgroundColor: "#4d2b95" },

  cardItem: { paddingLeft: 5, paddingRight: 5, paddingTop: 7, paddingBottom: 8, margin: 0, borderRadius: 20 },

  container: { flex: 1 },



});
